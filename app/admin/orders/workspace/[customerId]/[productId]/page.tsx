  ...
  const triggerMockUpload = async () => {
    ...
    try {
      const response = await fetch(`http://localhost:5000/api/deals/${supportDealId}/messages`, {
        ...
        body: JSON.stringify({ content: 'Shared revised launch checklist & configuration parameters.' }),
      });

      if (!response.ok) throw new Error('Failed');
      triggerToast('Attached checklist.');
    } catch (err) {
      console.error(err);
      triggerToast('Failed to attach checklist.');
    }
  };
            ...
            <div className="bg-white dark:bg-zinc-900/10 border border-zinc-200 dark:border-zinc-900 p-6 rounded-lg flex flex-col" style={{ height: '600px' }}>
              ...
              >
                ...
                <div className="flex justify-between items-center px-3 py-2 border-t border-zinc-100 dark:border-zinc-800 bg-zinc-50/60 dark:bg-zinc-900/60">
                  ...
                  >
                    <Paperclip className="w-3.5 h-3.5" /> Send Instructions
                  </button>
                  <button
                    type="submit"
