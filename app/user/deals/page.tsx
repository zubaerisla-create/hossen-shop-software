          ...
          {showRequestForm ? (
            ...
            deals.length === 0 ? (
              ...
              <div className="space-y-4">
                {/* Desktop Table View */}
                <div className="hidden lg:block overflow-x-auto bg-white dark:bg-zinc-950 border border-zinc-200 dark:border-zinc-900 rounded">
                        ...
                        <tr key={d.id} className="hover:bg-zinc-50/50 dark:hover:bg-zinc-900/20 transition-colors">
                          ...
                          <td className="p-4">
                            <div className="flex items-center gap-2">
                              ...
                              <div>
                                <span className="font-bold text-zinc-955 dark:text-white block">{d.title}</span>
                ...
                <div className="lg:hidden space-y-4">
                  {deals.map(d => (
                    ...
                    >
                      <div className="flex justify-between items-start">
                        <div>
                          <div className="flex items-center gap-2">
                            ...
                            <span className="font-mono text-[9px] font-bold text-zinc-400">{d.id}</span>
                          </div>
                          <span className="font-bold text-zinc-950 dark:text-white text-xs block mt-0.5">{d.title}</span>
                          <span className="text-[10px] text-zinc-550 block">{d.projectType}</span>
                        </div>
                        <span className={`px-2 py-0.5 rounded text-[8px] font-extrabold uppercase ${
                          d.status === 'Completed' || d.status === 'Delivered'
                            ? 'bg-emerald-50 dark:bg-emerald-500/10 text-emerald-600 dark:text-emerald-450'
                      ...
                      <div className="grid grid-cols-2 gap-2.5 text-[10px] border-t border-zinc-200/50 dark:border-zinc-800/50 pt-2.5 text-zinc-650 dark:text-zinc-400">
                        <div>
                          <span className="text-zinc-400 dark:text-zinc-500 uppercase text-[7px] font-bold block">Target Budget</span>
                          <span className="font-bold text-zinc-955 dark:text-white font-mono">{format(d.budget)}</span>
                        </div>
                        <div>
                          <span className="text-zinc-400 dark:text-zinc-500 uppercase text-[7px] font-bold block">Deadline</span>
                          <span>{d.deadline}</span>
                        </div>
                        <div>
                          <span className="text-zinc-400 dark:text-zinc-500 uppercase text-[7px] font-bold block">Priority</span>
                          <span className={`px-1.5 py-0.2 rounded text-[8px] font-bold uppercase w-fit block ${
                            d.priority === 'Urgent'
                        ...
                        <div>
                          <span className="text-zinc-400 dark:text-zinc-500 uppercase text-[7px] font-bold block">Progress</span>
                          <div className="flex items-center gap-1.5 mt-0.5">
                            <div className="w-16 h-1 bg-zinc-200 dark:bg-zinc-800 rounded overflow-hidden">
                              <div className="h-full bg-zinc-900 dark:bg-white" style={{ width: `${d.overallProgress}%` }}></div>
                            </div>
                            <span className="font-bold text-[8px]">{d.overallProgress}%</span>
                          </div>
                        </div>
                      </div>
